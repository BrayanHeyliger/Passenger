...
const driverCandidates: DriverCandidate[] = [
  { id: "driver-demo", name: "Demo Driver", vehicle: "Toyota Corolla", rating: 4.9, distanceMiles: 0.8, eta: "3 min", verified: true, paymentMethods: ["Efectivo", "Zelle", "Cash App"] },
  { id: "driver-luis", name: "Luis R.", vehicle: "Honda Civic", rating: 4.8, distanceMiles: 1.4, eta: "5 min", verified: true, paymentMethods: ["Efectivo", "Zelle", "Transferencia"] },
  { id: "driver-ana", name: "Ana G.", vehicle: "Nissan Versa", rating: 4.7, distanceMiles: 2.1, eta: "7 min", verified: true, paymentMethods: ["Efectivo", "Cash App", "PayPal"] },
];
    ...
    <main className="min-h-screen bg-[#071016] px-5 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-xl">
        ...
        <section className="mt-8 overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,#153137,#09161b)] p-6 shadow-2xl sm:p-8">
          ...
          <div className="mt-7 grid gap-3 rounded-2xl border border-white/10 bg-black/15 p-4 text-sm"><span className="flex gap-3"><MapPin className="mt-0.5 shrink-0 text-emerald-300" size={17} /><b>{trip.pickup}</b></span><span className="flex gap-3"><Navigation className="mt-0.5 shrink-0 text-rose-300" size={17} /><b>{trip.destination}</b></span></div>
          <div className="mt-5 grid grid-cols-[1fr_auto] items-center gap-3 rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4"><span><small className="block text-[10px] font-extrabold tracking-[.15em] text-emerald-200">RIDE SELECCIONADO</small><b className="mt-1 block text-lg">{trip.serviceLabel}</b><em className="mt-1 block text-xs not-italic text-white/60"><Clock3 className="mr-1 inline" size={13} />Llegada estimada: {trip.estimatedEta}</em></span><strong className="text-xl text-emerald-200">${trip.estimatedPrice}</strong></div>

          {selecting && <div className="mt-6 space-y-3"><div><p className="font-bold">Conductores disponibles cerca de ti</p><p className="mt-1 text-sm text-white/55">Elige el conductor que prefieras antes de enviar la solicitud.</p></div>{availableDrivers.map(driver => <button key={driver.id} onClick={() => selectDriver(driver, "manual")} className="w-full rounded-2xl border border-white/10 bg-white/[.055] p-4 text-left transition hover:border-emerald-300 hover:bg-emerald-300/10"><div className="flex items-start justify-between gap-3"><span><b className="block">{driver.name} <em className="ml-1 text-xs not-italic text-emerald-300">Verificado</em></b><small className="mt-1 block text-white/55">{driver.vehicle} · ★ {driver.rating.toFixed(1)} · {driver.distanceMiles.toFixed(1)} mi · {driver.eta}</small></span><ChevronRight className="text-emerald-200" size={19} /></div><span className="mt-3 flex flex-wrap gap-1.5">{driver.paymentMethods.map(method => <small key={method} className="rounded-full border border-white/10 bg-black/15 px-2 py-1 text-[11px] text-white/75">{method}</small>)}</span></button>)}{availableDrivers.length === 0 && <div className="rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4 text-sm text-amber-100">No hay conductores cercanos según las reglas actuales de operación. Ajusta radio, rating o métodos habilitados.</div>}</div>}

          {!selecting && !assigned && trip.driver && <div className="mt-6 rounded-2xl border border-white/10 bg-white/[.04] p-4"><p className="text-xs font-extrabold tracking-[.14em] text-emerald-300">{trip.assignmentMode === "auto" ? "AUTOBÚSQUEDA ACTIVA" : "CONDUCTOR ELEGIDO"}</p><div className="mt-3 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-emerald-300/15 text-emerald-200"><UserRoundCheck size={21} /></span><span><b className="block">{trip.driver.name}</b><small className="text-white/55">{trip.driver.vehicle} · ★ {trip.driver.rating.toFixed(1)} · {trip.driver.distanceMiles.toFixed(1)} mi</small></span></div><div className="mt-3 flex flex-wrap gap-1.5">{trip.driver.paymentMethods.map(method => <small key={method} className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-white/75">{method}</small>)}</div></div>}
          ...
          {assigned && <div className="mt-5 rounded-2xl border border-white/10 bg-white/[.04] p-4"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-emerald-300/15 text-emerald-200"><UserRoundCheck size={21} /></span><span><b className="block">{trip.driver?.name || "Conductor confirmado"}</b><small className="text-white/55">{trip.driver?.vehicle || "Vehículo asignado"} · llega en {trip.estimatedEta}</small></span></div></div>}
